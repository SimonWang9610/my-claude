...
1. **Enumerate behaviors for each surface.** List props, handlers, render branches, side-effects, and error paths.
   → read `references/heuristics-behavior-enumeration.md` (behavior enumeration grep recipes)
...
4. **Pass 3 — false-positive.** Scan for on-sight signals (no `expect()`, tautology, mock-shape drift, un-awaited write, CSS-class-for-color, mocked QueryClient). Confirm with the mutation-test mindset: invert or delete the deciding production branch — does the test still pass? Yes → false positive.
   ...
   → read `references/heuristics-pass3-forms.md` (forms 1–5 grep recipes + before/after examples, matcher misuse amplifiers)
...
- `references/heuristics-behavior-enumeration.md` — grep/read recipes for Pass 1 behavior enumeration.
- `references/heuristics-pass2-shapes.md` — shapes A–D with before/after examples for Pass 2.
- `references/heuristics-pass3-forms.md` — forms 1–5 grep recipes + before/after examples and matcher misuse amplifiers for Pass 3.
