...
- The workflow **generators** live at the repo root and are React-specific:
  ...
  phases directly to concrete `oac-*` skills (design-link / legacy-port / Jira / E2E
...
- `skills/` — one per phase concern: `oac-figma-decompose` (reuse/decompose a design source),
  ...
  task per AC), `oac-implementation` (correctness + React 19 idioms + performance behind a fixed
  contract), `oac-journey-tests` (author E2E tests from the approved plan), `oac-qa-report` (branch
  audit → sign-off report), and the paired `oac-test-contract` (authoring-time prevention) /
