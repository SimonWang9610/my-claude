...
lines, and gate verdict. Contracts are always one per unit.
...
4. **Check the design** — Run [references/challenges.md](./references/challenges.md): eight
   severity-tagged adversarial checks (C1–C8), best done by fresh eyes (a subagent given only the
   draft artifacts). Severity fixes the policy: **CRITICAL** must be re-designed (loop back to
   step 3), **HIGH** re-designs or carries a recorded justification, **MEDIUM** fixes or records
   debt, **LOW** is advisory. Repeat 3 ⇄ 4 until the hand-off criteria hold.
5. **Record** — Assemble `design.md` from [templates/design-doc.md](./templates/design-doc.md) and
