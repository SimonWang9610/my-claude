...
2. **WAIT for the user's instructions and context**, then run `/sf-init` to create the spec dir and `.meta.yaml` (owned by the project), then verify the spec dir exists and `.meta.yaml` is valid. If not, STOP and ask the user to run `/sf-init` first.
3. Determine the `<workflow-variant>` and then run `/sf-react-workflow <workflow-variant>` to generate the spec's `workflow.yaml` — BEFORE entering the Drive Loop. React only — the generator binds `oac-*` skills directly. When resuming a spec that already has a `workflow.yaml`, skip the generator and enter the Drive Loop at the first non-`completed` phase.
