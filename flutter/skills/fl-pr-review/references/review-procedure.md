...
of the `fl-architecture-design` skill (procedure and report formats in
`gate-procedure.md`, bundled here):
...
- Open `core/state-ownership-decision.md` — verify the
...
- Open `core/state-placement.md` — one owner per fact,
  derive don't duplicate, narrowest possible scope.
- Open `core/state-flow-and-async.md` — unidirectional
...
- Open `core/state-boundary-and-lifecycle.md` — every
  ...
  Only flag listenables the unit *owns* (created it); passed-in listenables are disposed by
  their owner — record ownership ambiguity as a low-confidence Minor, not a blocker.
- For Riverpod notifiers: `ref.watch` in `build()` only; `ref.read` only in event handlers
  ...
  with `@Riverpod(keepAlive: true)`); providers must be declared top-level (a provider inside
  a method, function body, or `State` class creates a new provider per build and leaks —
  Major). Flag UI consuming an `AsyncValue` via bare `.value` / `.requireValue` with no
  loading/error branch (should be exhaustive switch or `.when()`).

### Pass 3c — Data layer (repository + domain model)

For changed repositories and domain models:
- Open `core/repository-ssot.md` — single source of
  truth per type; DTO→domain mapping; owns caching/retry; no duplicate ownership.
- Open `core/domain-models-immutable.md` — immutable,
...
- Open `core/service-isolation.md` — raw DTOs only out
...
- Open `core/service-isolation.md` — one stateless
...
For changed widgets:
- Open `core/widget-composition.md` — `const
  StatelessWidget` classes, not `Widget _buildX()` helpers; default `StatelessWidget`.
- Open `core/widget-build-discipline.md` — `const`
  everywhere; small `build()`; no IO / business logic in `build()`; no `BuildContext` across
  an `await` without `mounted` check.
- Open `core/widget-theming.md` — colors/typography from
  `Theme.of(context)` tokens, not hard-coded values.
- General hygiene (Minor unless it causes a correctness bug): no `print` in production code
  (use a logger); no gratuitous `!` null assertions; keys on list items built from dynamic
  collections.

### Pass 3f — DI and composition root

For changes touching dependency wiring:
- Open `core/dependency-injection.md` — inject via
...
**fl-test-contract (six authoring-time rules from the `fl-test-contract` skill):**
...
**fl-test-forensics (three gap-class passes from the `fl-test-forensics` skill):**
...
Reference files for test forensics (bundled here under `forensics/`):
- `forensics/gap-classes.md`
- `forensics/heuristics-behavior-enumeration.md`
- `forensics/heuristics-pass2-shapes.md`
- `forensics/heuristics-pass3-forms.md`
- `forensics/false-positive-signals.md`
...
- **Performance pack (bundled here under `performance/`)** — activate ONLY when a
  ...
  - `performance/perf-rebuilds.md`, `performance/perf-build-cost.md`, `performance/perf-lists.md`,
    `performance/perf-images.md`, `performance/perf-animations.md`, `performance/perf-jank-and-startup.md`
...
- **`fl-riverpod` skill** — when the diff touches Riverpod-specific code (providers, `ref`,
  `ConsumerWidget`, `Notifier`, etc.), load the `fl-riverpod` skill
  ...
  / `AsyncNotifier`. Exception: if the touched file is already saturated with legacy providers
  and migration is clearly out of the PR's scope, downgrade to Minor with a
  follows-existing-legacy note. Use an analogous skill for other state-management packages.
...
is run. See `github-posting.md` for the full procedure — PR resolution, REST fallback,
inline-comment line mapping, and the preferred single batched review (summary body + verdict
event + inline comments in one review object).
