...
| P4 | Holder receives repo via constructor; exposes sealed state + named commands; package-agnostic seam | `DeviceNotifier(this._repo)` with `void select(String id)` command | `ChangeNotifier` injected with raw `Dio`; fetches, caches, paginates in one class | https://docs.flutter.dev/app-architecture/guide |
...
| P7 | Dispose every subscription/controller; sealed async type; never swallow errors | Sealed `DeviceState` union; `_sub?.cancel()` in `dispose()` | `bool isLoading + List? devices` (impossible state); leaked `StreamSubscription`; `catch (_) {}` | https://docs.flutter.dev/app-architecture/recommendations |
