...
- **Optional:** `preflight.md` (reuse + shared-widget context); prior-phase `references/`.
...
1. **Write user stories** — each with its acceptance criteria; when `preflight.md` surfaces existing widgets or screens, ground UI stories and their ACs in those real surfaces.
2. **Define functional requirements** — EARS notation (Ubiquitous / Event / State / Optional / Unwanted).
3. **List NFRs** — as `NFR-<n>`; Flutter-relevant NFRs include frame budget, startup time, offline behavior, and platform (iOS/Android/Web) parity.
4. **Apply the AC contract** — every AC/NFR gets a stable ID and observable Given/When/Then phrasing; reject implementation-step phrasing. Apply: fl-acceptance-criteria.

## Instructions & references

- [fl-acceptance-criteria](../skills/fl-acceptance-criteria/SKILL.md) — stable `AC-`/`NFR-` IDs, observability test, EARS validation.
