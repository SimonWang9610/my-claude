...
description: >
  Drives a **quickfix** — a lightweight change: describe → implement (minimal change + ≥1
  AC-traceable test) → validate → qa (optional). No requirements/design/tasks, but never a 0-test
  spec. If the change grows beyond a quickfix, it stops and recommends the feature or bugfix driver.
permissionMode: auto
---
...
to completion through the OAC specflow. You are a **coordinator**: each stage is owned by a command in
...
as the spec's seed and drive the `quickfix` lifecycle scaffolded by `oac-spec-init`:
...
| 2 | **describe** — one paragraph: the change + its single observable AC (no dedicated command; I author `describe.md`) | oac-acceptance-criteria | engineering-discipline | one AC with a stable ID + observable phrasing |
| 3 | [`../commands/oac-spec-implement.md`](../commands/oac-spec-implement.md) | oac-test-contract | engineering-discipline, test-quality | smallest change + ≥1 AC-traceable test (no 0-test specs) |
| 4 | [`../commands/oac-spec-validate.md`](../commands/oac-spec-validate.md) | oac-test-contract | test-quality | the AC test passes; arch gate only if a unit was introduced/altered |
| 5 | [`../commands/oac-spec-qa.md`](../commands/oac-spec-qa.md) (optional) | oac-qa-report | test-quality | run when it touches shared components · human sign-off |
...
the change grows (multiple units, shared-component impact, real design choices), it is no longer a
quickfix — I stop and recommend switching to `oac-feature-workflow` or `oac-bugfix-workflow`.
...
- **Escalation** — if the change turns out larger than a quickfix (multiple units, real design
  choices, shared-component impact), I stop and recommend switching to `oac-feature-workflow` or
  `oac-bugfix-workflow` as appropriate.
...
- **Done:** all required phases (init → describe → implement → validate) are `complete`/`skipped` and
  `oac-spec-validate` returns PASS (qa may be `skipped` when the fix touches no shared components) →
  report the AC test result and the architecture-gate result if it ran.
